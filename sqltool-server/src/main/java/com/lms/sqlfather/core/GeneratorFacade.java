...
public class GeneratorFacade {
    ...
    public static GenerateVO generateAll(TableSchema tableSchema) {
        ...
        // 生成模拟数据
        Map<String, List<Map<String, Object>>> generateAllData = DataBuilder.generateAllData(tableSchema);


        // 生成插入 SQL
        String insertSql = sqlBuilder.buildInsertSql(tableSchema, generateAllData);
        // 生成数据 json
        String dataJson = JsonBuilder.buildJson(generateAllData);
        // 生成 java 实体代码
        String javaEntityCode = JavaCodeBuilder.buildJavaEntityCode(tableSchema);
        // 生成 java 对象代码
        String javaObjectCode = JavaCodeBuilder.buildJavaObjectCode(tableSchema, generateAllData);
        // 生成 typescript 类型代码
        ...
        generateVO.setDataList(generateAllData);
...
//    }
    ...
    public static void validSchema(TableSchema tableSchema) {
        ...
        BusinessException.throwIf(tableSchema.getTableList().size()> SqlConstant.MAX_TABLE_NUM);
        tableSchema.getTableList().forEach(GeneratorFacade::validTable);
    }

    public static void validTable(TableSchema.Table table){

        String tableName = table.getTableName();
        if (StringUtils.isBlank(tableName)) {
            throw new SchemaException("表名不能为空");
        }
        Integer mockNum = table.getMockNum();
        // 默认生成 20 条
        if (table.getMockNum() == null) {
            table.setMockNum(20);
            mockNum = 20;
        }
