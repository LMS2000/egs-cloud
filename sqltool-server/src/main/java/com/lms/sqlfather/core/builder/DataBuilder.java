...
//    }
    ...
    //构造多表
  public static Map<String,List<Map<String,Object>>> generateAllData(TableSchema tableSchema){
      Map<String, TableSchema.Table> tableSchemaMap = tableSchema.getTableList().stream().collect(Collectors.toMap(TableSchema.Table::getTableName, Function.identity()));
      ...
      //初始化
      for (Map.Entry<String, TableSchema.Table> stringTableSchemaEntry : tableSchemaMap.entrySet()) {
          int mockNum = stringTableSchemaEntry.getValue().getMockNum();
          List<Map<String,Object>> dataList = new ArrayList<>(stringTableSchemaEntry.getValue().getMockNum());
          for (int i = 0; i < mockNum; i++) {
              dataList.add(new HashMap<>());
          }
          tableDataList.put(stringTableSchemaEntry.getKey(),dataList);
