 ...
 * @param params
 */
export async function updateCurrentUser(params: UserType.UserUpdateRequest) {
	return request<ResultData<any>>(`/user/update/current`, {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json',
		},
...
export async function getLoginUser() {
	return request<ResultData<UserType.UserVO>>('/user/get/login', {
		method: 'GET',
	});
}
/**
 * 上传用户头像
 */
export async function uploadUserAvatar(file: any) {
  const params = new FormData();
  params.append('file', file);
  return request<ResultData<any>>('/user/upload', {
    method: 'POST',
    headers: {
      'Content-Type': 'multipart/form-data',
    },
    data: params, // 使用 FormData 作为 body
  });
}
...
export async function flushKeys() {
  return request<ResultData<any>>('/user/flush/keys', {
    method: 'POST',
    headers: {
      	'Content-Type': 'application/json',
    }
